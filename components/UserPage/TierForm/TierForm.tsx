...
export const TierForm: React.FC<TierFormProps> = ({benefits, description, image, price, published, title, edit, setTiers, tiers, id}) => {
  const [tierImage, setTierImage] = useState<boolean>(false);
  const [publishedState, setPublishedState] = useState<boolean>(published);
  const [tierBenefits, setTierBenefits] = useState<{name: string, id: number}[]>(benefits ? benefits.split('|').map((item, index) => ({name: item, id: index})) : []);
        ...
        <Form>
            <div className="w-full px-6 py-7 border-xs border-gray3 border-opacity-30">
                <button className="mb-5 text-red underline text-opacity-80 text-xs text-right font-light flex flex-col">
                  {publishedState ? <span className="text-right items-end" onClick={() => setPublishedState(false)}>Unpublish</span> : <span onClick={() => setPublishedState(true)}>Publish</span>}
                </button>
                <Input
                  gap="gap-3 md:gap-2"
                  height="h-42px"
                ...
                <Input
                  ...
                  onEnter={() => {
                    setTierBenefits([...tierBenefits, {name: values.benefits, id: tierBenefits.length}]);
                ...
                <div className="flex flex-wrap gap-2 mb-5">
                  {tierBenefits.map(({id, name}, index) => (
                    name && <p className="px-2 py-1 bg-gray3 bg-opacity-10 rounded-4xl text-black text-opacity-60 text-xs flex items-center leading-none" key={index}>{name} <span onClick={() => setTierBenefits([...tierBenefits].filter((item) => item.id !== id))} className="ml-1 text-red text-10px inline-block cursor-pointer">X</span></p>
                  ))}
                </div>
                <TextArea
                  gap="gap-3 md:gap-0"
                ...
                <div className="flex flex-col lg:flex-row gap-2">
                  ...
                  <Button
                    ...
                    text="Cancel"
                    type="card1"
                    onClick={() => setTiers([...tiers].map(item => item.id === id ? {...item, edit: false} : {...item} ))}
                    className="mt-4 w-full lg:w-max"
                  />
                </div>
            </div>
      ...
      <div className="flex">
        <div className=" flex flex-col items-start flex-1">
          ...
          <div className="relative w-full h-24 mb-2 bg-gray7">
            <Image layout="fill" objectFit="cover" src={image ? image : '/placeholder.png'} alt='' />
          </div>
          ...
          <Button px="px-10 md:px-12" height='h-42px' onClick={() => {
            setTiers([...tiers].map(item => item.id === id ? {...item, edit: true} : {...item, edit: false} ))
          }} text='Edit Tier' type='card1' className="w-full"  />
        </div>
