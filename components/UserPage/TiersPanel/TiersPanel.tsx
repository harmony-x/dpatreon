...
export const TierPanel: React.FC = () => {
  ...
  const [tiers, setTiers] = React.useState<ExtendedTier[]>(
    Array<Omit<ExtendedTier, "id">>(3)
      .fill(defaultTiers)
      ...
      {isLoading && tiers ? (
        ...
        <div className="flex flex-col gap-6 md:gap-0 md:flex-row md:flex-wrap lg2:flex-nowrap lg2:gap-4">
          {tiers.map((item, index) => (
            <div
              className="w-full md:w-1/2 lg:w-2/5 lg2:w-1/3 md:pr-4 lg2:pr-0 lg:mb-4 lg2:mb-0"
              key={index}
            >
              <TierForm {...item} setTiers={setTiers} tiers={tiers} />
            </div>
          ))}
        </div>
      )}
