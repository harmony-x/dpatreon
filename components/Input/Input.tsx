...
export const Input : React.FC<InputProps> = ({label, onChange, placeholder, gap, labelFor, height, className, px, value, onEnter, isRequired = true, labelTextSize = 'lg'}) => (
  <div className={`relative mb-4 flex flex-col md:flex-row ${gap} justify-between ${className || ''}`}>
    <label htmlFor={labelFor} className="flex flex-col gap-1 w-full md:w-1/4">
      ...
      {isRequired ? <span className="text-xs md:text-sm text-black text-opacity-60 font-light">Required</span> : null}
    </label>
