...
const HowtoCard : FC<HowtoCardProps> = ({title, heading, description, Icon}) => {
  return (
    <article className="border pt-8 pb-20 px-8 w-full lg:w-1/3">
      ...
      <h5 className="font-medium text-lg md:text-2xl mb-2 text-black">{title}</h5>
      <p className="font-light text-sm md:text-base text-black text-opacity-90">{description}</p>
    </article>
  )
}
